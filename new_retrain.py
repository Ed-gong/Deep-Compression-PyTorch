...
if __name__=="__main__":
        ...
        num_thread = 4
        outdir = ""
        model = torch.load("/home/datalab/Deep-Compression-PyTorch/saves/model_after_retraining.ptmodel")
        ...
        for name, param in model.named_parameters():
            if 'weight' in name:
                ...
                #print(type(tensor))
                #print(tensor.shape)
                bias = torch.tensor(tensor, requires_grad=True)
        ...
        for name, param in model.named_parameters():
            if 'weight' in name:
                ...
                for i in range(len(sparse_tensor.row)): 
                    dd[edge_count] = (sparse_tensor.col[i], sparse_tensor.row[i], edge_count)
                    edge_count += 1
                    #print(sparse_tensor.row[i], sparse_tensor.col[i],sparse_tensor.data[i])
	...
	# train the network
        ...
        for epoch in range(100):
            ...
            for batch_idx, (data, target) in pbar:
                ...
                #print("is data contains NAN?", torch.isnan(data))
                #sys.exit()
